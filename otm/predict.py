...
from otm.data.dataloader import ALPHABET
...
def predict(args: argparse.Namespace):
    ...
    opt_temp_model = get_model(config["backbone"], config["model"], use_lora=config["use_lora"],
                               lora_rank=config["lora_rank"])
    load_model(args.model, opt_temp_model, None, with_opt=False)
    opt_temp_model.eval()

    results = make_predictions(opt_temp_model, pred_dataloader)
...
def make_predictions(opt_temp_model: nn.Module, pred_dataloader: DataLoader) -> dict:
    results = {"ids": [], "sequence": [], "prediction": []}
    with torch.no_grad():
        for b in pred_dataloader:
            output = opt_temp_model(b[2].to('cuda'))
