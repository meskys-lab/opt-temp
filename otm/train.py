...
from otm.data.dataloader import get_dataloaders
from otm.learner import get_learner
from otm.model.opt_temp_model import get_model
from otm.model.utils import freeze_backbone, unfreeze
...
def train(args):
    ...
    opt_temp_model = get_model(args.backbone, args.model, use_lora=args.use_lora, lora_rank=args.lora_rank)

    learner = get_learner(model=opt_temp_model, dataloaders=dataloaders)

    model_name = get_model_name(opt_temp_model)
...
def get_model_name(opt_temp_model: nn.Module) -> str:
    name = opt_temp_model.__class__.__name__
